...
public class AbstractBasicTest
{
    ...
    protected static ArrayList<String> methodNames;
    ...
    {
        try  {
            Field f = thisClass.getDeclaredField(fName);
            return f;
        }  catch (Exception e) {
            return null;
        }
    ...
    {
        try  {
            Field f = thisClass.getSuperclass().getDeclaredField(fName);
            return f;
        }  catch (Exception e) {
            return null;
        }
...
//                System.out.println("mName->" + mName + ", pos->" + pos + ", " + methodNames);
    ...
    protected void checkFieldModifiers(String name)
    {
        ...
        if (field == null)
            fail("SYNTAX: Field " + name + " is not found.");
        checkFieldModifiers(field);
    }
    
    protected void checkFieldModifiers(Field field)
    {
...
//            System.out.println("getMethod(name)->" + getMethod(name));
    ...
    protected void testInitialization(String name, Object expected, Object...args)
    {
        String desc = "--construct " + thisClass.getSimpleName() + 
                "(" + getParamDesc(args) + ")";
        try
        {
            obj = create(thisClass, args);
            Field f = getField(name);
            if (f == null) f = getSuperField(name);
            f.setAccessible(true);
            Object actual = f.get(obj);
            assertEquals("INCORRECT: " + name + " initialization: ", expected,
                    actual);
    ...
    /**
     * Run a test with a console program
     */
    protected void runWith(String...inputStr)
    {
        // CANNOT RUN UNLESS CLASS AND MAIN METHOD ARE CORRECT.
        if(thisClass == null || ! Modifier.isPublic(thisClass.getModifiers()))
            fail("Class NOT public. Can't test.");
        
        PrintStream oldOut = System.out;
        InputStream oldIn = System.in;
        
        ByteArrayOutputStream out = new ByteArrayOutputStream();
        EchoInputStream in = new EchoInputStream(inputStr);
        System.setOut(new PrintStream(out));
        System.setIn(in);
        final Object[] params = new Object[]{new String[]{}};
